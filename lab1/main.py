...
from tables.position_table import PositionsTable
...
class Main:
    ...
    def db_insert_somethings(self):
        ...
        pt.insert_one(["Test", "Test", "Test", 2])
        pt.insert_one(["Test2", "Test2", "Test2", 4])
        pt.insert_one(["Test3", "Test3", "Test3", 2])
        pht.insert_one([1, "123"])
        pht.insert_one([2, "123"])
        pht.insert_one([3, "123"])

        pos.insert_one(["sss", 232322, "sss1"])
        pos.insert_one(["dddd", 343232, "sssddd1"])
...
Основное меню (выберите цифру в соответствии с необходимым действием): 
    ...
    10 - просмотр должностной таблицы
    9 - выход.
    """
        print(menu)
        return
    ...
    def show_positions(self):
        self.person_id = -1
        menu = """Просмотр отчёта по должностям
№\tОтдел\tЗарплата\tДолжность"""
        print(menu)
        lst = PositionsTable().all()
        for i in lst:
            print(str(i[1]) + "\t" + str(i[0]) + "\t" + str(i[2]) + "\t" + "\t" + str(i[3]))
        menu = """Дальнейшие операции: 
        0 - возврат в главное меню;
        3 - добавление нового человека;
        4 - удаление человека;
